...
lexicographically, as in a dictionary (first by the first digit, then, if the first digit 
is equal, by the second, and so on). In addition, we do not consider an infinite set of 
natural numbers, but only the first n numbers.
...
// Answer:
// I started off with an empty array numOrder and did a for loop to push all n numbers in numOrder
// I used .map() to turn the elements into Strings, then sorted them to be in lexicographical order
// Then it used .map() again to store the elements as numbers
// I used indexOf in the index variable to get the index number of k
// Finally, I returned index + 1 since I started the for loop with a 1
function strangeMath(n, k){
  ...
  numOrder = numOrder.map(String).sort().map(Number);

  const index = numOrder.indexOf(k);
  return index + 1;
}